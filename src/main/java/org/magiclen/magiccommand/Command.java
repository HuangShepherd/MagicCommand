...
public class Command {
	...
	public static boolean sendSignal(final Signal signal, final int... pids) {
		...
		for (final int pid : pids) {
			try {
				final Process killProcess = Runtime.getRuntime()
						.exec("kill -".concat(String.valueOf(signal.value)).concat(" ").concat(String.valueOf(pid)));
				killProcess.waitFor();
	...
	public boolean inputStringToRunningProcess(final String id, final String inputString) {
		...
		try {
			...
			try (final BufferedWriter stdOut = new BufferedWriter(
